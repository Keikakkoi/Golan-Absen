...
export class ReportExportService {
  ...
  printStatisticsReport(title: string, printDate: string, summaryRows: unknown[][], detailHeaders: string[], detailRows: unknown[][]): void {
    const printWindow = window.open('', '_blank', 'width=1200,height=800'); if (!printWindow) return; const escape = (value: unknown) => this.escapeHtml(String(value ?? '-')); const logoUrl = new URL('assets/icon_golan.png', document.baseURI).href;
  ...
  private openPrintWindow(title: string, printDate: string, headers: string[], rows: unknown[][], alpha: boolean): void {
    ...
    printWindow.document.write(html); printWindow.document.close(); printWindow.opener = null;
  }
