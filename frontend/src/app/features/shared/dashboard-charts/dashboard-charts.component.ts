...
export class DashboardChartsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void { this.load(); this.disconnect = this.realtime.connectRealtime(() => this.load()); }
  ...
  load(): void {
    ...
    this.http.get<DashboardChartData>(`http://localhost:8080/api/v1/dashboard/charts${params}`, { headers: this.headers() }).subscribe({
