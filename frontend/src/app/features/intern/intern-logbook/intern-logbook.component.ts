...
export class InternLogbookComponent implements OnInit {
  ...
  edit(item: any): void { const status = item.status_logbook || item.StatusLogbook || 'draft'; if (status === 'approved' || status === 'submitted') return; this.editingId = item.id || item.ID; this.form = { tanggal: String(item.tanggal || item.Tanggal || '').slice(0, 10), tugas: item.tugas || item.Tugas || '', deskripsi_kegiatan: item.deskripsi_kegiatan || item.DeskripsiKegiatan || '', kendala: item.kendala || item.Kendala || '', status: status }; window.scrollTo({ top: 0, behavior: 'smooth' }); }
